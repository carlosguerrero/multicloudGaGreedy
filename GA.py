...
#******************************************************************************************
    ...
    def addVm(self,child):
        ...
        child['vminstances'].append(self.rnd.randint(0,len(self.system.vminstancesTypes)-1))
...
#******************************************************************************************
    ...
    def crossover(self,f1,f2,offs):
        ...
        c1 = copy.deepcopy(f1)
        c2 = copy.deepcopy(f2)
###INCORRECTO        cuttingPoints = self.rnd.sample(xrange(0,self.system.vmNumber+1),2)
        cuttingPoints = self.rnd.sample(xrange(0,min( len(c1['vminstances']) , len(c2['vminstances']) )+1),2)
    ...
    def calculateVmsWorkload(self, solution):
        ...
        for vmId in range(0,len(solution['vminstances'])):
            ...
            for msId in range(0,self.system.numberMicroServices):
                ...
                if solution['vmmsMatrix'][vmId][msId] == 0:
                    vmIdLoad += self.system.serviceTupla[msId]['storageResources']
...
#******************************************************************************************
    ...
    def runningInstanceCost(self, vm, solution):
        cost = self.system.vminstancesTypes[solution['vminstances'][vm]]['cost']['running']
        return cost

    def resourceUsageCost(self, vm, solution,vmsUsages):
        
        cost = self.system.vminstancesTypes[solution['vminstances'][vm]]['cost']['usage']
        capacity = self.system.vminstancesTypes[solution['vminstances'][vm]]['capacity']        
        use = vmsUsages[vm]
        finalCost = cost * (use / capacity)
        return finalCost

    def storedInstanceCost(self, solution):
        #las instancias almacenadas solo se pagaría una vez por cada tipo,y no una por instancia de vms.
        cost = 0.0
        usedInstances = set()
        for vmId in range(0,len(solution['vminstances'])):
            usedInstances.add(solution['vminstances'][vmId])
        for instanceId in usedInstances:
            cost+=self.system.vminstancesTypes[instanceId]['cost']['storage']
        return cost

#TODO completar los que faltaran del survey de Luigi.
    def calculateCost(self,solution,vmsUsages):
        totalCost = 0.0
        for vmId in range(0,len(solution['vminstances'])):
            totalCost += self.runningInstanceCost(vmId, solution) + self.resourceUsageCost(vmId, solution, vmsUsages) 
#las instancias almacenadas solo se pagaría una vez por cada tipo,y no una por instancia de vms.
#es elc oste que pagaríamos por tener almacenados en el provider cada uno de los templates de vm que tengo configurado para arrancar en un momento dado (tendría instalado kubernetes, etc...).
#por tanto, si usamos menos tipos de templates, el coste es menor.
        totalCost += self.storedInstanceCost(solution)
        return totalCost
    ...
    def calculateMttr(self,solution):
        ...
        for msId in range(0, self.system.numberMicroServices):  # para cada microservicio
            ...
            for vmId in range(0,len(solution['vminstances'])): #y para cada vm 
                if solution['vmmsMatrix'][vmId][msId] == 0:
                    storedVm.add(vmId)
                    storedProvider.add(self.system.vminstancesTypes[solution['vminstances'][vmId]]['provider'])
                if solution['vmmsMatrix'][vmId][msId] > 0:
                    runningVm.add(vmId)
                    runningProvider.add(self.system.vminstancesTypes[solution['vminstances'][vmId]]['provider'])
...
#******************************************************************************************
    ...
    def calculateLowerLatency(self, solution, originMsId, consumedMsId, originVm):
        ...
        originProvider = self.system.vminstancesTypes[solution['vminstances'][originVm]]['provider']
        ...
        for vmId in range(0,len(solution['vminstances'])): #recorro todas las vms
            if vmId != originVm:
                if solution['vmmsMatrix'][vmId][consumedMsId] > 0: #si esa vm tiene el ms consumido
                    currentProvider = self.system.vminstancesTypes[solution['vminstances'][vmId]]['provider'] #pillo el proveedor
        ...
        if latencyDistance == float('inf'): #esto lo hemos añadido porque si solo había una vm se devolvia infinito y también ocurría lo mismo si entre instancias de vms no había ms que consumían de la otra vm.
            latencyDistance = 0.0
...
#******************************************************************************************
    ...
    def calculateFitnessObjectives(self, pop, index):
        ...
        if self.checkConstraints(pop,index):
            chr_fitness["cost"] = self.calculateCost(chromosome,vmsUsages)
